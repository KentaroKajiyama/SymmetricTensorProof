...
-- -- -----------------------------------------------------------------------------
-- --  Transition Logic (1ステップ分の処理)
-- -- -----------------------------------------------------------------------------

-- /-あるグラフ g に対し、anchor と「forbidden リスト以外の頂点」を結ぶ可能なすべての辺の組み合わせを生成し、新しいグラフのリストを返す。-/
-- def transition_one {n} (g : AdjMat n) (anchor : Fin n) (forbidden : Array (Fin n)) : Array (AdjMat n) :=
--   let current_candidates :=
--     (List.finRange n).filter (fun v =>
--       v ≠ anchor ∧ !forbidden.contains v)
--   let new_graphs := current_candidates.map (fun target => g.add_edge anchor target)
--   new_graphs.toArray

-- /-グラフ集合全体に対して transition_one を適用し、結果を平坦化(flatten)する。-/
-- def transition {n} (graphs : Array (AdjMat n)) (anchor : Fin n) (forbidden : Array (Fin n)) : Array (AdjMat n) :=
--   graphs.flatMap (fun g => transition_one g anchor forbidden)

-- -- -----------------------------------------------------------------------------
-- --  Step Control (分割実行のための構造化)
-- -- -----------------------------------------------------------------------------

-- /-各ステップの実行に必要なパラメータ。どのアンカーに着目し、どの頂点への接続を禁止するか。-/
-- structure StepConfig (n : Nat) where
--   anchor : Fin n
--   forbidden : Array (Fin n)
--   all_anchors : Array (Fin n) -- reduce_iso で使用

-- /- ステップ番号 (1-based index) から、そのステップで行うべき構成(Config)を取得する関数。
--   Total 14 steps:
--     - Step 1-3:   Phase 1 (v1)
--     - Step 4-6:   Phase 2 (v2)
--     - Step 7-10:  Phase 3 (v3)  <-- ここで分割したい
--     - Step 11-14: Phase 4 (v4)
-- -/
-- def get_step_config {n} (step_i : Nat) (v_list : List (Fin n)) : Option (StepConfig n) :=
--   match v_list with
--   | [v1, v2, v3, v4] =>
--     let all := #[v1, v2, v3, v4]
--     let forbid (v : Fin n) := all.filter (· != v)
--     if 1 ≤ step_i ∧ step_i ≤ 3 then
--       some { anchor := v1, forbidden := forbid v1, all_anchors := all }
--     else if 4 ≤ step_i ∧ step_i ≤ 6 then
--       some { anchor := v2, forbidden := forbid v2, all_anchors := all }
--     else if 7 ≤ step_i ∧ step_i ≤ 10 then
--       some { anchor := v3, forbidden := forbid v3, all_anchors := all }
--     else if 11 ≤ step_i ∧ step_i ≤ 14 then
--       some { anchor := v4, forbidden := forbid v4, all_anchors := all }
--     else
--       none
--   | _ => none
...
-- /-【核心】1ステップ分だけ進める関数。
--   入力: 現在のグラフ集合
--   出力: 枝を張り、同型除去した後のグラフ集合-/
-- def run_step {n} (current_graphs : Array (AdjMat n)) (config : StepConfig n) : Array (AdjMat n) :=
--   -- 1. 枝を張る (Expand)
--   let expanded := transition current_graphs config.anchor config.forbidden
--   -- 2. 同型除去 (Reduce) -> NautyFFI を使用
--   reduce_iso expanded config.all_anchors
